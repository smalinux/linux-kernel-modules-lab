 ...
 * @author	smalinux
 *
 * Many people can hold a read lock, but a writer must be the sole holder.
 * Read-Write locks are more useful in scenarios where the architecture is
 * clearly divided into the reader and the writers, with more number of reads.
 *
 * In Read Write spinlock multiple readers are permitted at the same time
 * but only one writer. (i.e) If a writer has the lock, no reader is allowed
 * to enter the critical section. If only a reader has the lock, then
 * multiple readers are permitted in the critical section.
 */
